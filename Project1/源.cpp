...
public:
    ...
    bool checkCoinCollision(RECT otherRect) const {
        if (!hasCoin || coinCollected) return false;

        RECT coinRect = getCoinRect();
        return (otherRect.right > coinRect.left && otherRect.left < coinRect.right &&
            otherRect.bottom > coinRect.top && otherRect.top < coinRect.bottom);
    }
    ...
    bool hasCoinAvailable() const { return hasCoin && !coinCollected; }
    void collectCoin() { coinCollected = true; }
